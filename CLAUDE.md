...
**llama-server** runs in **router mode** — a built-in multi-model proxy, one router per GPU. The primary instance inherits `--models-max 1` from `.env`; `run.sh` overrides the secondary instance to `--models-max 2` so `gpt-oss-20b` and `qwen3-reranker-0.6b` remain resident together for Hindsight.
...
- `models-1.ini` is the Hindsight-oriented secondary router: `gpt-oss-20b` uses three slots and 300000 total context, while `qwen3-reranker-0.6b` uses eight slots with 32768 total context, 8192 batch/ubatch and `reranking = true`. Hindsight connects directly through its `litellm` reranker provider to llama.cpp's `/v1/rerank`; no adapter is needed.
- `gpt-oss-20b` uses `chat-template-file = templates/gpt-oss-20b-harmony.jinja`, the model's own template plus one added tool-channel instruction. Without it gpt-oss emits a malformed channel header on Reflect's later turns and llama.cpp returns HTTP 500. Re-extract and re-apply it after any gpt-oss model update. See HINDSIGHT.md.
...
- Preserve `--models-max 2` on the Vulkan1 router so the LLM and the reranker coexist, and keep exactly two models defined in `models-1.ini` — a third would evict one of them mid-workload. MTP remains disabled unless exact Hindsight end-to-end benchmarks prove it is not slower for the prompt-heavy workload.
